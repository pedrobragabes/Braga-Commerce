...
export function CheckoutForm({ storeSlug, settings }: CheckoutFormProps) {
  ...
  const firstMethod: CheckoutDeliveryMethod = settings.allowLocalPickup
    ? "LOCAL_PICKUP"
    : "LOCAL_DELIVERY";
  ...
  if (!items.length) {
    return (
      <div className="checkout-empty">
        ...
        <p>Adicione uma peça antes de preencher o checkout.</p>
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSubmitError(
        requestError instanceof Error ? requestError.message : "Não foi possível criar o pedido.",
      );
      setSubmitting(false);
    }
  }
  ...
  return (
    <form className="checkout-layout" onSubmit={handleSubmit}>
      <div className="checkout-forms">
        ...
        <fieldset className="form-section">
          ...
          <div className="field-grid">
            ...
            <label className="field">
              ...
              <input autoComplete="tel" minLength={8} name="phone" required type="tel" />
            </label>
            <label className="field">
              <span>
        ...
        <fieldset className="form-section">
          <legend>
            <span>02</span> Como receber
          </legend>
          <div className="delivery-options">
            {settings.allowLocalPickup ? (
              <label
                className={
                  deliveryMethod === "LOCAL_PICKUP" ? "delivery-card selected" : "delivery-card"
                }
              >
                <input
                ...
                <span>
                  ...
                  <small>Combine o horário após o pedido.</small>
                </span>
                <b>Grátis</b>
              </label>
            ) : null}
            {settings.allowLocalDelivery ? (
              <label
                className={
                  deliveryMethod === "LOCAL_DELIVERY" ? "delivery-card selected" : "delivery-card"
                }
              >
                <input
                ...
                <span>
                  ...
                  <small>Disponível na região atendida pela loja.</small>
                </span>
                <b>
                  {settings.localDeliveryFeeCents
                    ? formatCurrency(settings.localDeliveryFeeCents)
                    : "Grátis"}
                </b>
              </label>
            ) : null}
          ...
          {deliveryMethod === "LOCAL_DELIVERY" ? (
            <div className="field-grid address-grid">
              <label className="field">
                <span>CEP</span>
                <input autoComplete="postal-code" name="zipCode" required />
              </label>
        ...
        <fieldset className="form-section">
          <legend>
            <span>03</span> Observações
          </legend>
          <label className="field full">
            <span>
              Algum detalhe para a loja? <small>opcional</small>
            </span>
      ...
      <aside className="checkout-summary">
        <p className="section-eyebrow">Revisão final</p>
        <h2>{quote?.items.length ?? 0} item(ns)</h2>
        <div className="checkout-mini-lines">
          {quote?.items.map((item) => (
            <div key={`${item.productId}:${item.variantId ?? "simple"}`}>
              <span>
        ...
        <div className="summary-row">
          <span>{deliveryMethod === "LOCAL_DELIVERY" ? "Entrega local" : "Retirada"}</span>
          <strong>{shippingCents ? formatCurrency(shippingCents) : "Grátis"}</strong>
        </div>
        ...
        <button
          ...
          disabled={submitting || !quote || Boolean(quote.issues.length)}
          type="submit"
        >
          {submitting ? "Preparando pagamento…" : "Criar pedido e pagar"} <StoreIcon name="arrow" />
        </button>
        <p className="summary-note">
          <StoreIcon name="shield" size={17} /> Você pagará no ambiente seguro do Mercado Pago. O
          retorno ao site não confirma o pagamento; o pedido só muda após validação do webhook.
        </p>
      </aside>
    </form>
  );
