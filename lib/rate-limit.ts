...
export function createRateLimitKey(policyName: string, identifier: string, secret = getRateLimitSecret()) {
...
export async function checkRateLimit(request: Request, policy: RateLimitPolicy) {
  ...
  const key = createRateLimitKey(policy.name, getClientAddress(request));
  ...
  `);
  const bucket = rows[0];
  const retryAfter = Math.max(1, Math.ceil(((bucket?.expiresAt ?? expiresAt).getTime() - now.getTime()) / 1000));
...
export async function enforceRateLimit(request: Request, policy: RateLimitPolicy) {
  try {
    ...
    return Response.json(
      { error: { code: "PROTECTION_UNAVAILABLE", message: "Proteção temporariamente indisponível." } },
