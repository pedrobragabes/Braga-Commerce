...
describe("carrinho", () => {
  ...
  it("descarta rascunhos inválidos vindos do localStorage", () => {
    expect(sanitizeCartItems([{ productId: "p1", quantity: 2 }, { price: 10 }])).toEqual([
...
describe("checkout convidado", () => {
  ...
  it("exige endereço completo para entrega local", () => {
    ...
    if (!result.success)
      expect(result.error.issues.some((issue) => issue.path.join(".") === "address.zipCode")).toBe(
        true,
      );
  });

  it("normaliza telefone, CEP e UF e rejeita formatos incompletos", () => {
    const valid = checkoutRequestSchema.safeParse({
      ...base,
      customer: { ...base.customer, phone: "(16) 99999-9999" },
    ...
    if (valid.success) {
      expect(valid.data.customer.phone).toBe("16999999999");
      expect(valid.data.address?.zipCode).toBe("14000000");
      expect(valid.data.address?.state).toBe("SP");
    }
    expect(
      checkoutRequestSchema.safeParse({
        ...base,
        customer: { ...base.customer, phone: "12345678" },
      }).success,
    ).toBe(false);
  });
});
