...
describe("production operations", () => {
  it("chooses an explicit, Vercel, production or local public URL", () => {
    expect(getPublicAppUrl({ NEXT_PUBLIC_APP_URL: "https://loja.example/path" })).toBe(
      "https://loja.example",
    );
  ...
  it("bounds provider-controlled log fields and strips control characters", () => {
    expect(sanitizeLogContext({ requestId: "line\n" + "x".repeat(200) }).requestId).toBe(
      "line" + "x".repeat(156),
    );
  });
});
