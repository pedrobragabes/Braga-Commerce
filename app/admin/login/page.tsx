  ...
  return (
    <main className="admin-login">
      <section className="admin-login-story" aria-label="Braga Commerce">
        <Link className="admin-wordmark" href="/"><span>BC</span><strong>Braga Commerce</strong></Link>
        <div>
          <p className="admin-kicker">Caderno de operação · PV Moda</p>
          <h1>A loja aberta.<br />A operação em ordem.</h1>
      ...
      <section className="admin-login-panel">
        <form action={loginAdmin} className="admin-login-form">
          ...
          {error && errorMessages[error] ? <div className="admin-alert error" role="alert">{errorMessages[error]}</div> : null}
          <label><span>E-mail</span><input autoComplete="email" name="email" placeholder="voce@loja.com.br" required type="email" /></label>
          <label><span>Senha</span><input autoComplete="current-password" minLength={8} name="password" required type="password" /></label>
          <button className="admin-button primary" type="submit">Entrar na operação <span>→</span></button>
          <Link className="admin-back-link" href="/">← Voltar para a vitrine</Link>
        </form>
      </section>
    </main>
  );
